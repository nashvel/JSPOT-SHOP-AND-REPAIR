...
interface Props {
    ...
    categories?: Category[];
    mechanics?: Mechanic[];
    reservationMode?: boolean; // When true, adds items to reservation instead of sale
}

export default function MiniPosModal({ isOpen, onClose, saleId, currentPaymentMethod = 'cash', products, services = [], categories = [], mechanics = [], reservationMode = false }: Props) {
    const [searchQuery, setSearchQuery] = useState('');
    const [activeTab, setActiveTab] = useState<'products' | 'services'>('products');
    const [selectedCategory, setSelectedCategory] = useState<number | 'all'>('all');
    const [cart, setCart] = useState<{ product: Product; quantity: number; customPrice?: number }[]>([]);

    // Service price modal state
    ...
    const addToCart = (product: Product) => {
        // For services, open price input modal
    ...
    // Handle adding service from price modal
    const handleAddServiceFromModal = () => {
        ...
        if (existing) {
            setCart(cart.map(item =>
                item.product.id === selectedProduct.id
                    ? { ...item, quantity: modalQuantity, customPrice: modalPrice }
                    : item
            ));
        } else {
            setCart([...cart, {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // For reservations, no payment validation needed
        if (!reservationMode && data.amount_paid < cartTotal) {
            Swal.fire({
        ...
        // Prepare items with full info for reservations
        ...
        if (reservationMode) {
            ...
            // @ts-ignore - allowing extra fields for reservation items
            data.items = itemsData;
            post(route('admin.reservations.add-items', saleId), {
                onSuccess: () => {
                    onClose();
                    reset();
                    setCart([]);
                    Swal.fire({
                        title: 'Success!',
                        text: 'Items added to reservation.',
                        icon: 'success',
                        timer: 1500
                    });
                }
                        ...
                        <div className="p-3 bg-white border-t border-gray-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] space-y-3">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-3">
                                ...
                                <div className="flex gap-2 pt-1">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={processing || cart.length === 0 || (!reservationMode && data.amount_paid < cartTotal)}
                                    ...
                                    >
                                        {processing ? '...' : (
                                            reservationMode
                                                ? (data.amount_paid > 0 ? `Add & Pay ${formatPrice(data.amount_paid)}` : 'Add to Reservation')
                                                : (cart.length > 0 ? `Pay ${formatPrice(cartTotal)}` : 'Add Items')
                                        )}
                                    </button>
                                </div>
                            </form>
            ...
            {/* Service Price Modal */}
            {
                selectedProduct && (
                    <>
                        ...
                        />
                        <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm z-[70] p-4">
                            <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                                <div className="flex items-center justify-between p-4 border-b border-gray-100">
                                    <h2 className="text-lg font-semibold text-gray-900">Set Service Price</h2>
                                    <button
                                        onClick={() => setSelectedProduct(null)}
                                ...
                                <div className="p-4">
                                    <div className="flex items-start gap-4 mb-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h3 className="font-semibold text-gray-900 truncate">{selectedProduct?.name}</h3>
                                            <p className="text-xs text-gray-400">Base: {formatPrice(selectedProduct?.price ?? 0)}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="flex items-center justify-center gap-4">
                                            ...
                                            <button
                                                ...
                                                className="w-10 h-10 rounded-xl bg-indigo-500 hover:bg-indigo-600 flex items-center justify-center text-white"
                                            >
                                    ...
                                    <div className="flex items-center justify-between py-3 border-t border-gray-100 mb-4">
                                        ...
                                        <span className="text-xl font-bold text-gray-900">{formatPrice(modalPrice * modalQuantity)}</span>
                                    </div>

                                    <button
                                        onClick={handleAddServiceFromModal}
                                    ...
                                    >
                                        ...
                                        {cart.find(item => item.product.id === selectedProduct?.id) ? 'Update Cart' : 'Add to Cart'}
                                    </button>
                                </div>
                            </div>
                        </div>
